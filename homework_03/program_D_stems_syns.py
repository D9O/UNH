...
OR="OR"    #(same)
OPERATORS = { AND, OR } #the logical tokens we will use; how I use this should satisfy
                        #the stop words extra credit, I think...
...
#word stemming
STEMS = { "love": {"love", "loves", "loved", "loving", "lover"},
          "draw": {"draw", "drew", "drawing", "drawn"} }
#synonyms
SYNONYMS = { "love": {"deep affection", "fondness", "tenderness", "warmth", "intimacy",
                      "attachment", "endearment", "devotion", "adoration", "doting",
                      "idolization", "worship" },
             "draw": {"paint", "sketch", "illustrate", "diagram"} }
...
#NOTE: it only works for the actual query, NOT stems and synonyms.  It could be fixed
#by altering the return structure from AndOr_Eval to pass back the matches.
...
def Get_SynStems(term):
  syn_stems = set()
  if term != True or term != False:
    root_word = None
    for root, stems in STEMS.items():
      if term in stems:
        root_word = root
        syn_stems = syn_stems | stems
        break
    if root_word in SYNONYMS.keys():
      syn_stems = syn_stems | SYNONYMS[root_word]
  return syn_stems
...
#
#I added word stemming and synonyms for OR.  ANDing them is much harder than I expected
#and would require a rework of everything.  
def AndOr_Eval(terms, oper, quote):
  ...
  if oper == OR:
    for t in terms:
      ...
      if t == True:
        return True
      syn_stems = Get_SynStems(t)
      if len(syn_stems) > 0:
        for s in syn_stems:
          if s in quote:
            print("  synonym or word stem hit >{}<".format(s))
            return True
      if t in quote:
        return True
    return False
